({
    fetchRulesFromDB : function(component, searchkey) {
        ...
        component.set('v.ruleList',[]);
        var showSpinner = $A.get("e.c:ShowSpinner");
        showSpinner.fire();
        var isAsc = component.get('v.isAsc');
        var sortFieldName = component.get('v.selectedTabsoft');
        var fetchRulesFromDB = component.get("c.fetchAllRules");
        fetchRulesFromDB.setParams({
            'searchKey' : searchkey,
            'isAsc' : isAsc,
            'sortFieldName' : sortFieldName
        });
        fetchRulesFromDB.setCallback(this, function(response){
            ...
            if (state === "SUCCESS") {
                ...
                if(ruleList.length == 0) {
                    ...
                    console.log('fetchRulesFromDB currentTabSelectedRules' + JSON.stringify(currentTabSelectedRules));
                    console.log('fetchRulesFromDB currentSessionSelectedRules' + JSON.stringify(currentSessionSelectedRules));
                    console.log('First Rule ' + component.get('v.firstRule'));
                    var map = component.get('v.selectedRules');
                    console.log('fetchRulesFromDB selectedRules' + JSON.stringify(map));
                    if(map){
                        for(var i in ruleList){
                            ...
                            if(currentSessionSelectedRules){
                                currentRuleIndex = currentSessionSelectedRules.indexOf(rule.Rule_Library_Ext_Id__c);
                            }
                            var currentTabRuleIndex = -1;
                            if(currentTabSelectedRules){
                                currentTabRuleIndex = currentTabSelectedRules.indexOf(rule.Rule_Library_Ext_Id__c);
                            }
                            ...
                            if(tempRule && currentRuleIndex == -1){
                                ...
                                rule.ruledesc='';
                                rule.checked=false;
                                rule.operatorSelected=rule.Rule_Library_Ext_Id__c+'~none';
                                rule.operator='';
                                rule.isAnyRule='';
        ...
        $A.enqueueAction(fetchRulesFromDB);        
    },
    
    fetchRulesByCategory : function(component, filterBy, searchKey, isAsc, sortFieldName) {
        component.set('v.ruleList',[]);
        component.set('v.isShowGenericMessage',false);
        var showSpinner = $A.get("e.c:ShowSpinner");
        showSpinner.fire();
        var fetchRulesFromDB = component.get("c.fetchRulesByCategory");
        fetchRulesFromDB.setParams({
            'categoryLabel' : filterBy,
            'searchKey' : searchKey,
            'isAsc' : isAsc,
            'sortFieldName' : sortFieldName
        });
        fetchRulesFromDB.setCallback(this, function(response){
            ...
            if (state === "SUCCESS") {
                ...
                if(ruleList.length == 0) {
                    ...
                    if(map){
                        ...
                        for(var i in ruleList){
                            ...
                            if(tempRule){
                                ...
                                rule.operatorSelected=tempRule.operatorSelected;
                                rule.operator=tempRule.operator;
                                rule.operatorButton=tempRule.operatorButton;
                                rule.isAnyRule=tempRule.isAnyRule;
                                rule.anyRuleOperator=tempRule.anyRuleOperator;
                                rule.anyRuleValue=tempRule.anyRuleValue;
                            }else{
                    ...
                    //Code Snippet added for persisting the selected rules from the search on the ANY OF table.
                    var anyOfSelectedRules = component.get('v.anyOfSelectedRules');
                    if(anyOfSelectedRules){
                        for(var i in ruleList){
                            var rule = ruleList[i];
                            var anyChecked = anyOfSelectedRules[rule.Rule_Library_Ext_Id__c];
                            console.log('AnyChecekd: '+anyChecked);
                            if(anyChecked){
                                rule.anyChecked = true;
                            }else{
                                rule.anyChecked = false;
                            }
                        }
                    }
    ...
    fetchTypesFromDB : function(component) {
        var fetchAllTypes = component.get("c.fetchTypesData");
        console.debug('Fetching for types');
        fetchAllTypes.setCallback(this, function(response){
            ...
            if (state === "SUCCESS") {
                ...
                var selectedFilter = component.get('v.ruleListFilter');
                var filterBy = component.find('selectFilterBy');
                var isAsc = component.get('v.isAsc');
                var sortFieldName = component.get('v.selectedTabsoft');
                filterBy.set('v.value', selectedFilter);
                if(selectedFilter && selectedFilter != 'All' ) {                    
                    this.fetchRulesByCategory(component,selectedFilter,'',isAsc,sortFieldName);                    
                }
                else {
                    this.fetchRulesFromDB(component);       
                }                
            }
        });
    ...
    removeSelectedRuleById : function(component, helper, ruleId){
        console.log('Rule Id to be removed: '+ruleId);
        var map = component.get("v.selectedRules");
        ...
        if(tempObj){
            ...
            component.set('v.overlayRules',overlayRules);
            console.log('overlayRules after delete');
        ...
        /*var selectedRulesforANY = component.get('v.selectedRulesforANY'); 
        for(var i=0;i<selectedRulesforANY.length;i++){
            var rTemp = selectedRulesforANY[i];
            console.log('ANY OF Rules :' + rTemp.ruleId );
            if(rTemp && rTemp.ruleId==ruleId){   
                
                selectedRulesforANY.splice(i,1);
            }
        }
        ...
        //}
        //cmpEvent.setParams({"ICombinationString" : combinationConstructed, "ICombinationMap":map, "ICombinationANYMap":selectedRulesforANY, "IOverlayRules":overlayRules, "IShowANYOFTable" : showAnyOverlay, "IShowRuleListTable": true}).fire();
    ...
    completeAnyOfCombination : function(component, event, helper){
        console.log('inside completeAnyOfCombination');
        var map = component.get('v.selectedRules');
        var cashedObj=component.get('v.anyCashedObject');
        var tempLast = map[cashedObj.ruleId];
        var anyOperator = component.get("v.anyOperator");
        var anyValue = component.get("v.anyValue");
        // var btn = event.getSource();
        if(anyOperator && anyValue){
            if(anyOperator!='none' && !isNaN(anyValue)){
                console.log('in operaotr if');
                tempLast.startOfAny=false;
                var i = component.get("v.anyIndex");
                tempLast.anyIndex=i;
                tempLast.endOfAny=true;
                tempLast.anyOperator=anyOperator;
                tempLast.anyValue=anyValue;
                i = i + 1;
                component.set("v.anyIndex", i);
                component.set('v.selectedRules',map);
                ...
                var cmpEvent = component.getEvent("ConstructCombination");
                console.log('Combination Created: '+combinationConstructed);
                cmpEvent.setParams({"ICombinationString" : combinationConstructed, "ICombinationMap":map, "ICombinationANYMap":selectedRulesforANY}).fire();
            }
        }else{
            // btn.set("v.errors","Either Operator/Value is not selected");
            console.log('Either Operator/Value is not selected');
        }
    },
    
    reconstuctAnyOfExpression : function(map,ruleId){
        var trashObj = map[ruleId];
        if(trashObj){
            if(trashObj.startOfAny){
                var nextRuleId = trashObj.nextElement;
                var newStart = map[nextRuleId];
                if(newStart){
                    newStart.preElement=null;
                    newStart.startOfAny=true;
                    newStart.anyOfMember=true;
                }
                trashObj.startOfAny=false;
                trashObj.anyOfMember=false;
                
            }else if(trashObj.endOfAny){
                var preRuleId = trashObj.preElement;
                var newEnd = map[preRuleId];
                if(newEnd){
                    newEnd.nextElement=null;
                    newEnd.endOfAny=true;
                    newEnd.anyOperator=trashObj.anyOperator;
                    newEnd.anyValue=trashObj.anyValue;
                }
                trashObj.endOfAny=false;
                trashObj.anyOfMember=false;
                trashObj.anyOperator=null;
            }else if(trashObj.anyOfMember){
                var nextRuleId = trashObj.nextElement;
                var preRuleId = trashObj.preElement;
                var nextObj = map[nextRuleId];
                var preObj = map[preRuleId];
                if(preObj){
                    preObj.nextElement=trashObj.nextElement;
                }
                if(nextObj){
                    nextObj.preElement=trashObj.preElement;
                }
                trashObj.anyOfMember=false;
            }
        }
    ...
    refreshRuleListHelper : function(component, helper, isAnyRuleFlag){
        ...
        if(map){
            for(var i=0;i<ruleList.length;i++){
                ...
                var rule = ruleList[i];
                var tempObj = map[rule.Rule_Library_Ext_Id__c];
                //console.log('Rule Operator: ' + rule.operator + ' ANY Operator ' + rule.anyOperator);
        ...
        var overlayRules = component.get('v.overlayRules');
        var currentSessionSelectedRules = component.get('v.currentSessionSelectedRules');
        cmpEvent.setParams({"ICombinationString" :combinationConstructed, "ICombinationMap":map, "IOverlayRules":overlayRules, "IShowANYOFTable" : false, "IShowRuleListTable": true, "ICurrentSessionSelectedRules": currentSessionSelectedRules}).fire();        		
    },
    ...
    highlightFirstANYRule : function(component, map, ruleList){        
        for(var mapRow in map){
            ...
            if(rules.length > 1){
                for(var i=0;i<ruleList.length;i++){
                    var rule = ruleList[i];
                    if(rule.Rule_Library_Ext_Id__c == rules[0]){
                        console.log(rules[0]);
    ...
    sortHelper: function(component, event, sortFieldName) {
	  ...
	  var isAsc = component.get('v.isAsc');
      this.fetchRulesByCategory(component, filterBy, searchKey, isAsc, sortFieldName);
   },
})